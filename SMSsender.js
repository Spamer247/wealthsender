...
const messageContent = fs.readFileSync('letter.txt', 'utf8'); // You can change this to 'letter.html' if needed
...
function sendEmail(to, subject, message) {
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) {
      console.error(chalk.red(`❌ SMS sent to ${to} - Error: ${error}`));
      console.log(chalk.yellow(`┌───────────────────────┐`));
      console.log(chalk.yellow(`│ SMS SENT: Error ❌       │`));
      ...
      console.log(chalk.yellow(`┌───────────────────────┐`));
      console.log(chalk.green(`│ SMS SENT: Successful ✅        │`));
      console.log(chalk.yellow(`│ FROM: ${senderName}   │`));
      console.log(chalk.yellow.bold(`│ CODER NAME: xxxwealthxx │`));
      console.log(chalk.yellow(`└───────────────────────┘`));
    }
...
console.log(chalk.yellow.bold('💌 Sending SMS... 💌\n'));
